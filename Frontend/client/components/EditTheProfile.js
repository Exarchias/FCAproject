...
function EditTheProfile(prop) {
  ...
  const handleSubmitEdi = (event) => {
    ...
    var formi = document.getElementById("edittheprofile"); 
    var passEdi2 = formi.passEdi.value;

// Find user login info
const userDataEdi = prop.userSupervised;
...
const errorsEdi = {
  ...
  //=========== EDIT USER MECHANISM AREA CODE ENDS HERE =======================
    ...
    return (
        <section className='edittheprofile' style={{display: prop.panel =="edittheprofile" ? 'block' : 'none'}}>
            <div className="form">
            <h1>Edit {prop.userSupervised.username}</h1>
            <hr />
